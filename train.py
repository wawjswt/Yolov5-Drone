...
def train(hyp, opt, device, callbacks):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        pbar = enumerate(train_loader)  # 枚举
...
def parse_opt(known=False):
    ...
    # 模型，初始化的模型，可以为空。采用已经训练好的模型作为初始化输入能够提高训练速度，迁移学习，模型架构
    parser.add_argument("--cfg", type=str, default="models/yolov5s-C3GC-E.yaml", help="model.yaml path")
    # 模型的配置，
    ...
    # 四边形训练方式，以往是正方形，现在只要是最小的32的倍数就行，长宽不必相同
    parser.add_argument("--resume", nargs="?", const=True, default=False, help="resume most recent training")
    # 从之前训练的epoch开始，传入模型路径
